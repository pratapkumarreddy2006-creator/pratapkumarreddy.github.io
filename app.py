...
def add_to_cart():
    try:
        ...
        if not product_id:
            return jsonify(
                {'status': 'error', 'message': 'Product ID required'}
            ), 400
...
def place_order():
    ...
    try:
        ...
        for field in required_fields:
            if field not in customer or not str(
                    customer[field]).strip():
                return jsonify({
                    'status': 'error',
        ...
        # Convert cart dict to list if needed
        # (from {product_id: qty} to list format)
        cart_items = []
